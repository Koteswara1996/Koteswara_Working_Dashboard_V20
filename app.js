...
const app = {
    ...
    renderListManagerItems() {
        ...
        (this.lists[this.editingListKey] || []).forEach((item, index) => {
            container.innerHTML += `<div class="list-manager-item" style="display:flex; justify-content:space-between; align-items:center; padding:10px 12px; font-size:0.88rem; border-bottom:1px solid var(--separator);"><span>${this.sanitize(item)}</span><button type="button" class="btn-row btn-modal danger" style="padding:4px 10px; height:auto; color:var(--red-ink); background:rgba(239, 68, 68, 0.1); border:1px solid rgba(239, 68, 68, 0.2); border-radius:8px; cursor:pointer;" data-action="list-delete" data-index="${index}">Delete</button></div>`;
        });
    },
    ...
    renderDashboard() {
        ...
        const scopeSel = document.getElementById('dashGlobalCategory');
        const scope = scopeSel ? scopeSel.value : 'All';
        if (scopeSel) scopeSel.classList.toggle('active-filter', scope !== 'All');

        let live = this.tasks.filter(t => !t.deleted && !t.purged);
        if (scope !== 'All') live = live.filter(t => (t.category || '') === scope);
        ...
        if (greetBox) {
            ...
            if (scope !== 'All') line += ' Showing <b>' + this.sanitize(scope) + '</b> only.';

            greetBox.innerHTML = '<h1>' + part + who + '</h1><p>' + line + '</p>';
        }
    ...
    filterFromDashboard(ftype, fvalue) {
        ...
        // Keep the dashboard's category scope when drilling into something else.
        if (scope !== 'All' && ftype !== 'category') this.setMultiValue('filterCategoryOpts', scope);

        this.switchTab('Register');

        const labels = { due: 'Due', category: 'Category', status: 'Status', priority: 'Priority', pending: 'Pending with' };
        const pretty = { Today: 'Due today', Overdue: 'Overdue', Next7Days: 'Next 7 days', NoDue: 'No due date' };
